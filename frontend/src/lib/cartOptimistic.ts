...
export function patchCartLineQuantity(cart: Cart, productId: string, cantidad: number): Cart {
...
export function removeCartLine(cart: Cart, productId: string): Cart {
  const productos = cart.productos.filter((item) => item.producto._id !== productId);
  const subtotal = productos.reduce((acc, item) => acc + safeMoney(item.subtotal), 0);
  const descuentos = safeMoney(cart.descuentos);
  const impuestos = safeMoney(cart.impuestos);
  const costoEnvio = safeMoney(cart.costoEnvio);
